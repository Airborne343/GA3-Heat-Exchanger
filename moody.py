...
cold_poly_coeffs = np.polyfit(cold_flow, cold_pressure, 3)
hot_poly_coeffs = np.polyfit(hot_flow, hot_pressure, 3)
...
# Optional: plot fits
x_cold = np.linspace(0, max(cold_flow)*1.2, 200)
x_hot = np.linspace(0, max(hot_flow)*1.2, 200)
